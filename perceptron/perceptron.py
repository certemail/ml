...
#data_set = [[2,6,-1],[1,3,1],[3,1,1]]    # linearly separable
...
if __name__ == '__main__':
    ...
    for i in range(MAX_ITER):
        ...
        for x_input_vector in data_set:
            ...
            # compute error (ya) and update weights and bias only if wrong 
            if (y_actual * activation) <= 0:
                for k in range(D):
                    weights[k] = weights[k] + (y_actual * x_input_vector[k])
                bias = bias + y_actual
                print("    MISCLASSIFIED --> updated weights and bias are now: " + str(weights) + ", bias = " + str(bias))
                NUM_WRONG += 1
